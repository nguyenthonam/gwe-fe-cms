...
export default function BillRecipientInfo({ form, onChange, disabled }: Props) {
  return (
    <Box mb={2} bgcolor="#fff" p={2} borderRadius={2} boxShadow={1}>
      ...
      <Field label="Name" value={form.recipient?.fullname || ""} onChange={(v) => onChange("recipient", "fullname", v)} required={!disabled} disabled={disabled} />
      ...
      BillProductInfo <Field label="City" value={form.recipient?.city || ""} onChange={(v) => onChange("recipient", "city", v)} required={!disabled} disabled={disabled} />
      ...
      <CountrySelect value={form.recipient?.country?.code} onChange={(v) => onChange("recipient", "country.code", v || "")} required={!disabled} />
    </Box>
  );
}
