...
import { createVolWeightRateApi } from "@/utils/apis/apiVolWeightRate";
...
export default function CreateVolWeightRateDialog({ open, onClose, onCreated }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await createVolWeightRateApi({
        carrierId,
        supplierId,
        value: Number(value),
      });
      showNotification("Tạo tỉ lệ quy đổi thành công", "success");
      onCreated();
    } catch (err: any) {
      showNotification(err.message || "Lỗi tạo VolWeightRate", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle>Tạo Tỉ Lệ Quy Đổi</DialogTitle>
      <DialogContent>
