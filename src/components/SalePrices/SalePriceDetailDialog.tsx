...
/* ========= Sub component: PriceTable (có thu gọn/mở rộng) ========= */
...
type SalePriceDetailDialogProps = {
  open: boolean;
  group: ISalePriceGroup | null;
  onClose: () => void;
  /** Callback xoá toàn bộ group (bảng giá bán) */
  onDeleteGroup?: (group: ISalePriceGroup) => Promise<void> | void;
};

const SalePriceDetailDialog: React.FC<SalePriceDetailDialogProps> = ({ open, group, onClose, onDeleteGroup }) => {
  ...
  const docRows = docWeights.map((w) => [
    formatNumberVi(w), // cân nặng kiểu Việt Nam
    ...docZones.map((z) => {
  ...
  const handleDeleteAll = async () => {
    ...
    const confirmed = window.confirm("Are you sure you want to delete the entire sale price table?");
    if (!confirmed) return;
      ...
      <DialogActions sx={{ background: "#fafbfc", borderTop: "1px solid #e0e0e0" }}>
        {onDeleteGroup && (
          <Button variant="outlined" color="error" onClick={handleDeleteAll} disabled={deleting}>
            Delete entire sale price table
          </Button>
        )}
...
export default SalePriceDetailDialog;
