...
interface WeightRange {
  min: number;
  max: number;
  label: string; // giữ label gốc, nhưng render sẽ dùng min/max
}

interface ParsedTable {
  weightRanges: WeightRange[];
  zones: number[];
  prices: number[][]; // số thật để submit
}
...
export default function CreateSalePriceDialog({ open, onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    if (!open) {
      ...
      setDateError("");
      setOpenSuccess(false); // ✅ reset luôn popup success khi đóng Dialog
      return;
    }
  ...
  const parseExcelTable = (text: string) => {
    ...
    if (isSingle) {
      ...
      for (let i = 0; i < weightRanges.length; i++) {
        ...
        const label = lines[startLine + i].trim().split(/\t| +/)[0]; // giữ label gốc
  ...
  const handleSubmitAll = async () => {
    ...
    try {
      ...
      // reset input table để user có thể paste bảng mới
      setExcelInput("");
      setParsedTable(null);

      // ✅ mở popup xác nhận thành công (Modal riêng)
      setOpenSuccess(true);
    } catch (err: any) {
  ...
  return (
    <>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={6}>
                <FormControl fullWidth size="small">
                  ...
                  <Select label="Customer" value={partnerId} onChange={(e) => setPartnerId(e.target.value)}>
                    {partners?.map((p) => (
      ...
      <MuiDialog open={openSuccess} onClose={() => setOpenSuccess(false)}>
        ...
        <DialogContent>
          <Typography>Batch sale price has been created successfully.</Typography>
        </DialogContent>
        <DialogActions>
          <Button
            onClick={() => {
